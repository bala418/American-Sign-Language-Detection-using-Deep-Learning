...
def validate(model, dataloader):
    ...
    with torch.no_grad():
        for i, data in tqdm(
            enumerate(dataloader), total=int(len(test_data) / dataloader.batch_size)
        ):
        ...
        val_accuracy = 100.0 * running_correct / len(dataloader.dataset)
...
def fit(model, dataloader):
    ...
    train_accuracy = 100.0 * running_correct / len(dataloader.dataset)
